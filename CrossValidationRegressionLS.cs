...
namespace ConsoleApp3
{
    ...
    /// 
    /// </summary>
    class CrossValidationRegressionLS
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        double crossvalidate(double[][] x, double[] y, double[] payoff0, double[] payoff1,List<Func<double[], double>> bf, double crossValFactor = 0.1, int runs = 5, bool useAll = false)
        {
            if (x.Length != y.Length)
                throw new ArgumentException("Arrays x and y must be of same size.");
            ...
            for (int r = 0; r < runs; ++r)
            {
                ...
                var crit = forAll.Select(i => weights[i] == 0 ? reg.Result(x[i]) - y[i] : 0).Select(a => a * a).Sum() / weights.Sum();
                mses.Add(crit);
            }
        ...
        /// <param name="allFunc"></param>
        /// <returns></returns>
        int addFunction(double[][] x, double[] y,  double[] payoff0, double[] payoff1 , List<int> funcUse, List<Func<double[], double>> allFunc, bool useAll = false)
        {
            var abf = takeFunc(funcUse, allFunc);
            double actualcross = funcUse.Count() == 0 ? double.PositiveInfinity : crossvalidate(x, y,payoff0, payoff1, takeFunc(funcUse, allFunc));
            ...
            for (int i = 0; i < allFunc.Count(); i++)
                if (!funcUse.Contains(i))
                {
                    ...
                    double mse = crossvalidate(x, y, payoff0, payoff1, fu, useAll: useAll);
        ...
        public CrossValidationRegressionLS(double[][] x, double[] y, Func<double[], double>[] f, double[] payoffsP1, double[] payoffP0, double[] weights = null, bool useAll = true)
        {
            this.f = f;
            ...
            while (true)
            {
                int next = addFunction(x, y, payoffP0, payoffsP1, basisFunctionIndex, f.ToList(), useAll: useAll);
